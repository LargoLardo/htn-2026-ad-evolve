  ...
  return <Dialog open={!!candidate} onClose={onClose} label="Creative inspector">
    {candidate && <div className="grid grid-cols-1 gap-6 md:grid-cols-[1fr_1.2fr]">
      <div className="flex flex-col gap-3">
        ...
        <p className="text-xs text-foreground-lighter">{neural?.source === 'tribe-percept' ? 'TRIBE evaluated this media using Percept’s scoring method.' : review ? 'Automated media review is available below.' : 'No Percept evaluation has been recorded.'}</p>
      </div>
      <div className="flex flex-col gap-4">
        ...
        {neural?.source === 'tribe-percept' && <section className="flex flex-col gap-3" aria-label="Percept neural scores">
          <h3 className="label">Percept overall · {score(neural.engagementScore)} / 100</h3>
