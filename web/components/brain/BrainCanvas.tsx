...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    float level = dot(mask, uFamilyLevel) * uActivityLevel;

    vec3 color = mix(shaded, tint, smoothstep(0.0, 0.4, level)) + tint * level * 0.35;
...
export default function BrainCanvas({
  ...
  return (
    <Canvas camera={{ position: [2.7, 0.55, 1.5], fov: 35 }} dpr={[1, 2]}>
      <Cortex geometry={mesh} levels={levels} colors={colors} activityLevel={activityLevel} />
      <OrbitControls
        ref={controls}
        makeDefault
        enableDamping
