...
import { DEFAULT_GRID, disagreements, gap, normalize } from './grid.mjs';
...
export async function buildMaps(asset, brief, {
  signal, baseline, contractHash, grid = DEFAULT_GRID,
  heatmapPath, impactMetric = DEFAULT_IMPACT_METRIC,
} = {}) {
  ...
  try {
    impact = await buildImpactMap(asset, brief, { signal, baseline, contractHash, grid });
  } catch (caught) {
    signal?.throwIfAborted();
    error = caught.message;
  }

  if (!impact) return { grid, attention, impact: null, impactError: error, gap: null };
