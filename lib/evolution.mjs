...
export function validateBrief(input) {
  ...
  if (mode !== 'live' || scorer !== 'tribe') throw new Error('The previous heuristic/demo scorer has been removed. Use live generation with neural scoring.');
...
export const hasNeural = candidate => candidate.scores.neural?.source === 'tribe-neural';
...
// different scales and the difference is meaningless. The neural
...
//
// Only a shortlist ever reaches neural scoring, so a run always contains both kinds.
...
export async function evolveRun(run, providers, { signal, onUpdate = async () => {}, chooseParents, isInterruption = () => false } = {}) {
  ...
  const evaluateTribe = async candidates => {
    ...
    for (const candidate of pending) {
      ...
      if (!neural || neural.source !== 'tribe-neural' || !Number.isFinite(neural.engagementScore) || neural.engagementScore < 0 || neural.engagementScore > 100 || neural.baselineHash !== baseline.hash || neural.baselineMediaHash !== baseline.mediaHash || neural.contractHash !== baseline.contractHash || !neural.provenance || result.mediaHash !== candidate.asset.mediaHash) throw new Error('TRIBE returned invalid neural scores or media identity.');
      ...
      candidate.scores = { ...candidate.scores, source: 'tribe-neural', neural, metadata: result.metadata || {} };
      cache.set(fingerprint(candidate), { asset: candidate.asset, scores: candidate.scores });
      if (result.cached) run.metrics.cacheHits++;
  ...
  try {
    ...
    if (!capabilities.tribe) throw new Error('Neural scoring requires the updated TRIBE scoring endpoint. Decoder training is paused.');
    ...
    for (let round = 1; round <= brief.rounds; round++) {
      ...
      if (original && !neuralBaseline) {
        ...
        await update('scoring', 'Evaluating the uploaded original once to establish the fixed neural baseline.');
      ...
      current.scoreKind = 'tribe-neural';
      current.selectedIds = parents.map(candidate => candidate.id);
      current.evaluated = order.length;
      await update('evolving', `Generation ${round}: retained the strongest candidate and selected ${parents.length} parents using the highest neural overall scores against the shared original baseline.`);
    ...
    await update('complete', `Finished ${brief.rounds} generations. ${run.requiresReview ? 'No drafts passed the media checks; the retained provisional drafts need review and revision.' : 'Finalists ranked by neural overall score against the original creative. Scores are experimental cortical proxies, not measured emotion or ad effectiveness.'}`);
  } catch (error) {
