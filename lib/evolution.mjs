...
export async function evolveRun(run, providers, { signal, onUpdate = async () => {}, chooseParents, isInterruption = () => false } = {}) {
  ...
  try {
    ...
    run.requiresReview = run.finalists.some(candidate => candidate.provisional || candidate.scores.review?.copyWarning);
    await render(run.finalists);
    signal?.throwIfAborted();
    run.status = 'completed';
    await update('complete', `Finished ${brief.rounds} generations. ${run.requiresReview ? 'Finalists ranked by neural overall score; some have unresolved copy differences or media-review issues to inspect before use.' : 'Finalists ranked by neural overall score against the original creative. Scores are experimental cortical proxies, not measured emotion or ad effectiveness.'}`);
  } catch (error) {
