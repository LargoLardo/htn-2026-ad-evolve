 ...
 *
 * The unmasked original is scored in the same batch rather than assumed to be
 * 50. It is only exactly 50 when it is itself the run's baseline media, and
 * reading it back costs one pass we would otherwise spend guessing.
 */
export async function buildImpactMap(original, brief, { signal, baseline, contractHash, grid = DEFAULT_GRID } = {}) {
  ...
  const candidates = [{ id: 'occl-original', asset: original }, ...masked.map(item => item.candidate)];
  const scored = await scoreTribe(candidates, brief, { signal, baseline, contractHash });
  const byId = new Map(scored.results.map(result => [result.id, result.neural]));
  ...
  return {
    grid, calls: scored.calls, baseline: scored.baseline,
