 ...
 *
 * Occlude one element of the ad, rescore with TRIBE, and measure how far the
 * score moved. A large drop means that element was carrying the response.
 *
 * This is deliberately NOT an attention map. The `attention_salience`
...
import { impactResult } from './impact-result.mjs';
 ...
 */
export async function buildImpactMap(original, brief, { signal, baseline, contractHash, elements = [], onProgress, concurrency = Number(process.env.TRIBE_CONCURRENCY || process.env.PERCEPT_CONCURRENCY) || 1 } = {}) {
  ...
  // Raise concurrency only after confirming active_replica > 1.
  const byId = new Map();
  ...
  if (!originalNeural) throw new Error('TRIBE did not score the unmasked original.');
  return impactResult(original, resolvedBaseline, originalNeural, masked.map(({ cell, candidate }) => {
    const neural = byId.get(candidate.id);
    if (!neural) throw new Error(`TRIBE did not score the occluded "${cell.label}".`);
    return { cell, asset: candidate.asset, neural };
  }), calls);
}
