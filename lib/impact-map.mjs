 ...
 */
export async function buildImpactMap(original, brief, { signal, baseline, contractHash, grid = DEFAULT_GRID, onProgress, concurrency = Number(process.env.PERCEPT_CONCURRENCY) || 6 } = {}) {
  ...
  //
  // Concurrency is bounded by the deployment's replica count, not by us being
  // polite: each replica has concurrency_target 1, so more in-flight requests
  // than replicas just queue. `concurrency` should track max_replica.
  const byId = new Map();
