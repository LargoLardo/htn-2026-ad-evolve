...
async function scoreTribe(candidates, brief, { signal, contractHash, baseline } = {}) {
  ...
  const keyFor = (payload, reference) => `neural-${hash(stable([contract.hash, endpoint, payload.media_hash, reference?.hash || 'self']))}`;
  const evaluate = async (items, reference) => {
    const response = await (neuralRequest || requestJson)(endpoint, { action: 'neural', contract_hash: contract.hash, candidates: items, baseline: reference || null }, { signal, token, provider: 'TRIBE worker', timeout: 900_000, retries: 4 });
    calls++;
