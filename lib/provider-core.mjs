...
export function validateScreen(result, mediaHash) {
  ...
  // reviewer evidence stays intact for the user to inspect before using an ad.
...
async function screenCandidate(candidate, brief, { signal } = {}) {
  ...
  const saved = await cachedJson(key);
  // Eligibility is recomputed from cached evidence, including older reviews
  // that rejected strong visuals solely for a copy mismatch.
