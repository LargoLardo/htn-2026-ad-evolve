...
export async function renderCandidate(candidate, brief, { signal } = {}) {
  ...
  if (brief.mediaType === 'video') {
    const videoStyle = brief.referenceStyle ? ` Art direction to match: ${brief.referenceStyle}` : '';
  ...
  const prompt = `Create a polished square image advertisement. ${description}${style}`;
