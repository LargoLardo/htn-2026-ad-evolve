 ...
 */
export async function describeElements(asset, { signal } = {}) {
...
export async function describeReferences(assets, { signal } = {}) {
...
export async function readImpactNotes(asset, { signal } = {}) {
  abort(signal);
  if (asset?.mediaType !== 'image' || !asset.mediaHash) return '';
  const file = resolve(process.env.EVOLVE_MAPS_DIR || 'data/maps', `${asset.mediaHash}-maps.json`);
  let artifact;
...
export async function scoreTribe(candidates, brief, { signal, contractHash, baseline } = {}) {
  ...
  const evaluate = async (items, reference) => {
    const response = await requestJson(endpoint, { action: 'percept', contract_hash: contract.hash, candidates: items, baseline: reference || null }, { signal, token, provider: 'Percept/TRIBE worker', timeout: 900_000, retries: 4 });
    calls++;
