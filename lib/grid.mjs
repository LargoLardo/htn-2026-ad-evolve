...
/**
 * Map resolution. 8x8 rather than 3x3 because a 3x3 map cannot point at
 * anything smaller than a third of the ad, which is too coarse to say "the
 * racquet" rather than "the bottom left". Resolution is close to free: the
 * occluder slides, so the call count depends on `DEFAULT_WINDOW`, not this.
 */
export const DEFAULT_GRID = 8;

/** How many cells wide the occluder is, in cell units. See `windows`. */
...
/**
 * Sliding occluder positions: a `k`x`k` block of cells, stepped one cell at a
 * time across an `n`x`n` grid.
 *
 * This exists because map resolution and occluder size pull in opposite
 * directions. Raising the grid to get a finer map shrinks each occluder, and a
 * 1/64th patch of an ad moves the Percept score by less than the run-to-run
 * noise, so a 8x8 map of single cells is finer AND emptier. Zeiler and Fergus
 * hit the same wall and solved it the same way: keep the occluder big enough to
 * matter, move it in small steps, and let the overlaps localise the signal.
 *
 * Cost is (n-k+1)^2 passes, not n^2. An 8x8 map with a 3-cell occluder is 36
 * passes rather than 64, and each pass hides 1/9th of the ad instead of 1/64th.
 */
export function windows(width, height, n = DEFAULT_GRID, k = DEFAULT_WINDOW) {
  if (!Number.isInteger(k) || k < 1 || k > n) throw new Error('Occluder must be an integer from 1 cell up to the grid size.');
  const region = cells(width, height, n);
  const at = (row, col) => region[row * n + col];
  const out = [];
  for (let row = 0; row + k <= n; row++) {
    for (let col = 0; col + k <= n; col++) {
      const first = at(row, col), last = at(row + k - 1, col + k - 1);
      const covers = [];
      for (let r = row; r < row + k; r++) for (let c = col; c < col + k; c++) covers.push(r * n + c);
      out.push({ row, col, index: out.length, covers,
        x: first.x, y: first.y, w: last.x + last.w - first.x, h: last.y + last.h - first.y });
    }
  }
  return out;
}
...
/**
 * Spread each window's measured delta back over the cells it covered.
 *
 * A cell's value is the mean delta of every window that hid it. Interior cells
 * are hidden by more windows than corner cells, so dividing by the actual count
 * rather than k*k keeps the edges on the same scale instead of darkening them.
 */
export function accumulate(deltas, region, cellCount) {
  if (deltas.length !== region.length) throw new Error('Expected one delta per occluder position.');
  const sums = new Float64Array(cellCount), counts = new Float64Array(cellCount);
  region.forEach((window, index) => {
    for (const cell of window.covers) { sums[cell] += deltas[index]; counts[cell] += 1; }
  });
  return Array.from(sums, (sum, index) => (counts[index] ? sum / counts[index] : 0));
}
...
export function impactNotes(elements, { limit = 3 } = {}) {
  ...
  const name = element => element.label;
  const parts = [];
  if (dead.length) parts.push(`In the original, these drew the eye without moving the predicted response, so do not reproduce their treatment: ${dead.map(name).join('; ')}.`);
  if (working.length) parts.push(`These carried the response, so keep what they were doing: ${working.map(name).join('; ')}.`);
  return `Measured from the original ad. ${parts.join(' ')}`;
}
