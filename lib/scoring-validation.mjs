...
export function validateNeural(result, candidate, baseline, contract) {
  ...
  if (result?.id !== candidate.id || result.media_hash !== candidate.media_hash || n?.source !== 'tribe-neural' || n.contractHash !== contract.hash || n.baselineHash !== baseline.hash || n.baselineMediaHash !== baseline.mediaHash || !Number.isFinite(n.engagementScore) || n.engagementScore < 0 || n.engagementScore > 100 || !Number.isInteger(n.frames) || n.frames < 1 || n.frames > 120 || n.regions?.length !== 4 || !n.provenance) throw new Error('Invalid neural score, contract, baseline or media identity.');
  if (new Set(n.regions.map(r => r.key)).size !== 4) throw new Error('Invalid neural family scores.');
  for (const region of n.regions) {
    if (!FAMILY_KEYS.includes(region.key) || !Number.isFinite(region.score) || region.score < 0 || region.score > 100 || region.values?.length !== n.frames || region.values.some(v => !Number.isFinite(v) || v < 0 || v > 100)) throw new Error('Invalid neural family scores.');
  }
  // Optional: older cached scores predate per-parcel traces, so absence is fine.
  if (n.parcels !== undefined) {
    if (!Array.isArray(n.parcels) || !n.parcels.length) throw new Error('Invalid neural parcel traces.');
    for (const parcel of n.parcels) {
      if (!FAMILY_KEYS.includes(parcel.key) || typeof parcel.name !== 'string' || !parcel.name || parcel.values?.length !== n.frames || parcel.values.some(v => !Number.isFinite(v) || v < 0 || v > 100)) throw new Error('Invalid neural parcel traces.');
    }
  }
  if (n.version !== contract.version || result.metadata?.protocol !== contract.protocol || result.metadata?.neural_revision !== contract.neural_revision || !/^[a-f0-9]{64}$/.test(result.metadata?.prediction_hash) || !Number.isFinite(n.duration) || n.duration <= 0 || n.global?.length !== n.frames || n.global.some(v => !Number.isFinite(v) || v < 0 || v > 100)) throw new Error('Invalid TRIBE worker protocol or time series.');
