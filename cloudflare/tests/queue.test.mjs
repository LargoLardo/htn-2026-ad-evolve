...
import { consumePercept } from '../src/percept-queue.mjs';
...
async function setup() {
  ...
  const body = { accountId, runId: run.id, workflowId: 'test-workflow', kind: 'evolution', eventType: 'percept-test', jobId: sha256('job') };
  ...
  const options = { ...env, EVOLUTION: binding, TRIBE_SCORE_URL: 'https://percept.test/predict' };
...
it('streams one stimulus, validates its baseline and reuses saved results on redelivery', async () => {
  ...
  const first = message(body); await consumePercept({ messages: [first] }, options);
  ...
  const duplicate = message(body); await consumePercept({ messages: [duplicate] }, options);
  ...
  const again = message(body); await consumePercept({ messages: [again] }, options);
...
it('waits for the requested active replica count before sending any stimulus', async () => {
  ...
  options.PERCEPT_CONCURRENCY = '4'; options.BASETEN_API_KEY = 'TEST';
  const calls = [];
  ...
  const pending = message(body); await consumePercept({ messages: [pending] }, options);
