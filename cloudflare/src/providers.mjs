...
export function cloudProviders(env, accountId, { step, checkpoint } = {}) {
  ...
  return createProviders({ env, ...media, ...store, getScoringContract,
    ...
    async perceptRequest(endpoint, payload, { token, signal }) {
      if (payload.candidates.length !== 1) throw new Error('Percept requests must contain exactly one stimulus.');
      ...
      if (!response.ok) throw new Error(`Percept returned HTTP ${response.status}.`);
