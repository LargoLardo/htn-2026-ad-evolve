...
//
//   node scripts/build-demo-maps.mjs --image ad.png [--out data/maps]
//
// Cost is one worker call per detected element, plus one for the unmasked
// original, so about 13 rather than the 37 a grid needed. Each call is roughly
// two minutes on one L4 replica at concurrency 1. That is fine to pay once and
// unacceptable to pay on stage, so the demo reads this file instead of calling
// the worker.
...
if (!imagePath) { console.error('Usage: node scripts/build-demo-maps.mjs --image <file.png> [--out data/maps]'); process.exit(1); }
...
console.log(`${basename(imagePath)}  ${asset.width}x${asset.height}  ${asset.mediaHash.slice(0, 12)}`);
console.log('One worker call per detected element, plus one for the unmasked original.\n');
...
await writeFile(path, JSON.stringify({
  mediaHash: asset.mediaHash, width: asset.width, height: asset.height,
...
if (result.gap) {
  const worst = result.elements[0];
  console.log(`Widest gap: ${worst.label}  ${worst.gap >= 0 ? '+' : ''}${worst.gap.toFixed(2)}  ` +
